...
class CriteriaHelper
{
     ...
     */
    public function generateListWebsitePostCriterion(Location $location, array $viewParameters, array $languages = array())
    {
        $criteria = array();
        ...
        if (!empty($viewParameters))
        {
            if (!empty($viewParameters['month']) && !empty($viewParameters['year']))
            {
                ...
                $criteria[] = new Criterion\DateMetadata(
                        Criterion\DateMetadata::CREATED, Criterion\Operator::BETWEEN, array(
                    $date->getTimestamp(),
